...
// Wasmd 0.70.x still imports the removed SDK x/params types solely for its
// historical Wasm v2 -> v3 migration. The local replacement is compile-only;
// the target v0.55 app does not mount or register x/params.
replace github.com/cosmos/cosmos-sdk/x/params => ./third_party/cosmos-sdk-x-params-compat

// Wasmd v0.70.3 targets SDK v0.54. Its production x/wasm code is compatible
// with v0.55, but two legacy test-helper aliases are not. This pinned local
// compatibility snapshot contains only those two source-level adjustments.
replace github.com/CosmWasm/wasmd => ./third_party/wasmd-v055-compat

// WasmVM v3 decodes engine metrics through shamaton/msgpack v2. The latest
// tagged v2 release panics on truncated extension frames. Pin the upstream v2
// backport merged in shamaton/msgpack#66 until a fixed v2 tag is published.
...
require (
	...
	github.com/cosmos/cosmos-sdk/x/params v0.0.0-00010101000000-000000000000 // indirect
