...
- Preserve existing consensus and module parameter values during migration. Initialize the new governance fields with `min_deposit_ratio=0`, `proposal_cancel_ratio=0` and expedited minimum deposit of 110,000 DORA. The expedited voting period is one day and threshold is 0.667; preflight checks compatibility with existing governance parameters.
...
- Do not overwrite this tag or its artifacts if fixes are needed; publish a new version. After the upgraded binary commits state, do not start 0.4.4 against the same database.
