...
func TestMsgSetSponsor(t *testing.T) {
	...
	require.Equal(t, sdkmath.NewInt(1000000), msg.MaxGrantPerUser[0].Amount)
...
func TestMsgUpdateSponsor(t *testing.T) {
	...
	maxGrant := sdk.NewCoins(sdk.NewCoin("peaka", sdkmath.NewInt(500000)))
	...
	require.Len(t, msg.MaxGrantPerUser, 1)
	require.Equal(t, "peaka", msg.MaxGrantPerUser[0].Denom)
	require.Equal(t, sdkmath.NewInt(500000), msg.MaxGrantPerUser[0].Amount)
	...
	t.Run("max grant", func(t *testing.T) {
		require.NotPanics(t, func() {
			_, err := NormalizeMaxGrantPerUser([]*sdk.Coin{
				...
				{Denom: SponsorshipDenom, Amount: sdkmath.OneInt()},
			})
			require.Error(t, err)
...
func TestValidateGenesis_MaxGrant_InvalidDenomOrAmount(t *testing.T) {
	...
	gen := NewGenesisState([]*ContractSponsor{{
		...
		SponsorAddress:  sp,
		IsSponsored:     true,
		MaxGrantPerUser: []*sdk.Coin{{Denom: "dora", Amount: sdkmath.NewInt(1)}},
	}}, nil)
	err := ValidateGenesis(*gen)
	require.Error(t, err)
	...
	gen = NewGenesisState([]*ContractSponsor{{
		...
		SponsorAddress:  sp,
		IsSponsored:     true,
		MaxGrantPerUser: []*sdk.Coin{{Denom: "peaka", Amount: sdkmath.NewInt(0)}},
	}}, nil)
	err = ValidateGenesis(*gen)
	require.Error(t, err)
...
func TestValidateGenesis_UserGrant_InvalidCoins(t *testing.T) {
	...
	gen := NewGenesisState(nil, []*UserGrantUsage{{
		UserAddress:     user,
		ContractAddress: contract,
		TotalGrantUsed:  []*sdk.Coin{{Denom: "dora", Amount: sdkmath.NewInt(1)}},
	}})
	err := ValidateGenesis(*gen)
	require.Error(t, err)
	require.Contains(t, err.Error(), "invalid denomination")
...
func TestValidateGenesis_UserGrant_MayExceedCurrentLimit(t *testing.T) {
	...
	usages := []*UserGrantUsage{{
		...
		ContractAddress: contract,
		TotalGrantUsed:  []*sdk.Coin{{Denom: "peaka", Amount: sdkmath.NewInt(101)}},
	}}
