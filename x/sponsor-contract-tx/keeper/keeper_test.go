...
// TestGetSponsorsByStatus tests the GetSponsorsByStatus function
func TestGetSponsorsByStatus(t *testing.T) {
	keeper, ctx := setupKeeperSimple(t)
	
	// Set up test sponsors
	activeSponsors := []types.ContractSponsor{
		{
			ContractAddress: "dora1contract1",
			IsSponsored:     true,
		},
	...
	inactiveSponsors := []types.ContractSponsor{
		{
			ContractAddress: "dora1contract3",
			IsSponsored:     false,
		},
	...
	for _, sponsor := range inactiveSponsors {
		keeper.SetSponsor(ctx, sponsor)
	}
	
	// Test active sponsors
	activeSponsorsList := keeper.GetSponsorsByStatus(ctx, true)
	require.Len(t, activeSponsorsList, 2)
	
	// Test inactive sponsors
	inactiveSponsorsList := keeper.GetSponsorsByStatus(ctx, false)
	require.Len(t, inactiveSponsorsList, 1)
	require.False(t, inactiveSponsorsList[0].IsSponsored)
}

// TestGetSponsorCount tests the GetSponsorCount function
func TestGetSponsorCount(t *testing.T) {
	keeper, ctx := setupKeeperSimple(t)
	
	// Initially should be 0
	count := keeper.GetSponsorCount(ctx)
	require.Equal(t, uint64(0), count)
	
	// Add sponsors with proper bech32 addresses
	sponsors := []types.ContractSponsor{
		{ContractAddress: sdk.AccAddress([]byte("contract1___________")).String(), IsSponsored: true},
		{ContractAddress: sdk.AccAddress([]byte("contract2___________")).String(), IsSponsored: false},
		{ContractAddress: sdk.AccAddress([]byte("contract3___________")).String(), IsSponsored: true},
	}
...
// TestGetActiveSponsorCount tests the GetActiveSponsorCount function
func TestGetActiveSponsorCount(t *testing.T) {
	keeper, ctx := setupKeeperSimple(t)
	
	// Initially should be 0
	count := keeper.GetActiveSponsorCount(ctx)
	require.Equal(t, uint64(0), count)
	
	// Add sponsors (2 active, 1 inactive) with proper bech32 addresses
	sponsors := []types.ContractSponsor{
		{ContractAddress: sdk.AccAddress([]byte("contract1___________")).String(), IsSponsored: true},
		{ContractAddress: sdk.AccAddress([]byte("contract2___________")).String(), IsSponsored: false},
		{ContractAddress: sdk.AccAddress([]byte("contract3___________")).String(), IsSponsored: true},
