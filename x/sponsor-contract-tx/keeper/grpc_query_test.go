...
func (suite *GRPCQueryTestSuite) SetupTest() {
	...
	suite.maxGrant = sdk.NewCoins(sdk.NewCoin("peaka", sdkmath.NewInt(1000)))
}
			...
			preRun: func() {
				...
				for i, contractAddr := range contracts {
					sponsor := types.ContractSponsor{
						...
						MaxGrantPerUser: coinsToProtoCoins(sdk.NewCoins(sdk.NewCoin("peaka", sdkmath.NewInt(1000)))),
					}
					err := keeper.SetSponsor(freshCtx, sponsor)
					suite.Require().NoError(err)
				}
...
func (suite *GRPCQueryTestSuite) TestQuerySponsorBalance() {
	...
	coins := sdk.NewCoins(sdk.NewCoin(types.SponsorshipDenom, sdkmath.NewInt(123)))
