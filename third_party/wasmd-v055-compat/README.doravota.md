...
Pinned upstream commit: `47e9cb8aa080eba5db31bc294f614a2e15b3c82e`.
Maintained by Dora Vota; release reviewer to be assigned. See
`upstream-diff.patch` and `upstream-files.json` for the exact changes and
omitted source files. Local snapshots are not covered by `go mod verify`;
`snapshot-sha256.json` records every included file.
