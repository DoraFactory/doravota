...
func TestGroupCreateQueryAndGenesisRoundTripSDK055(t *testing.T) {
	chainApp := policyTestApp(t)
	...
	ctx := chainApp.BaseApp.NewNextBlockContext(
		tmproto.Header{
			Height:  1,
	...
	restored := policyTestApp(t)
	restoredCtx := restored.NewNextBlockContext(tmproto.Header{Height: 1, Time: ctx.BlockTime()})
	restored.GroupKeeper.InitGenesis(restoredCtx, restored.AppCodec(), restored.AppCodec().MustMarshalJSON(exported))
	require.Equal(t, exported, restored.GroupKeeper.ExportGenesis(restoredCtx, restored.AppCodec()))
}
